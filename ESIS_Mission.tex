...
\section{ESIS Mission}
	...
	\subsection{Pointing} \label{sec:point}
		...
		In fact, the last  B-class event detected by GOES \citep{GOES} prior to the ESIS launch was July 7th 2019 \jdp{figure out how to cite GOES, also, I couldn't believe I had to go back so far so check me on this}.  
...
%       	\includegraphics{}
    ...
    Level-3 data takes a cropped single wavelength from Level 1 data and maps to the sky plane via a non-linear coordinate transform, but wavelength independent mapping. 
    The highest level data product we envision, Level 4, is an $[x, y , \lambda]$ data cube, which is the result of an inversion. Since inversion is not unique, there may be multiple L4 products obtained by different methods.
...
\section{Preliminary Results}
    ...
    \subsection{SMART}
    	...
    	With SMART we seek to reconstruct the true spatial-spectral cube, $I_{xy\lambda}$, using every ESIS image, $f^{\theta_i}_{x'y'}$,=. There is one ESIS image for each $\theta_i$ representing the relative orientations of each ESIS channel. The procedure is executed as follows:
    	\begin{enumerate}
