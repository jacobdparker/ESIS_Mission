...
\cck{Nitpicky things: I've been working on fixing units (always a thin space between the number and the unit). Fix any you see! I fixed up the typesetting of the spectralline macro as well. Also the arcmin and arcsec commands are not appropriately defined (should use ensuremath) and are not uniformly used in the text; I recommend simply using primes (not backslash prime, but the apostrophe within math mode), which is less effort.}

\section{Introduction}

    \cck{Observationally,} the solar transition region (TR) refers to the portion of the solar atmosphere \cck{\sout{that emits}} at temperatures between 20,000\,K and 800,000\,K \cck{\sout{degrees}}. 
    ...
    \cck{[Moved to following paragraph] One solution to simultaneously capture spatial and spectral information over a large field of view is to use a slitless imaging spectrograph.  
    ...
    To a limited extent, it proved possible to isolate individual line intensities for bright spectral features such as flare kernels using the Skylab data \citep{TBD}. 
    Slitless spectrographs using two projections (usually one dispersed, and one undispersed) have proven sufficient to implement an efficient ground-based magnetograph \citep{deforest2004}, map Doppler shifts in the solar transition region \citep{Courrier2018}, and invert temperature or density information \citep{winebarger2019}. 
    However, as with any tomographic inversion problem \citep{Kak_Slaney}, the fidelity of reconstruction improves dramatically as projections are added. 
    ...
    An instrument that captures multiple projections of the spatial-spectral data cube is called a Computed Tomography Imaging Spectrograph (CTIS)\cite{Descour,Dereniak}.  
    MOSES is an example of a CTIS, as it captured three projections in the $\pm$ 1 and 0 orders.  \cite{Fox10} was able to extract line widths and doppler shifts from a relatively complex explosive event observed by MOSES in \heii.  \cite{Rust2019} found unambiguous evidence of explosive events with well resolved, double-peaked spectral line profiles in the same data set.
...
In this section we provide an overview of the ESIS experiment as well as the time and conditions of the ESIS rocket launch and subsequent data collection.   
    ...
    % 	operated in frame-transfer mode. Details like this are in the instrument paper 
    	...
    	\amy{Reminder to add line list and discuss relative brightness/degree of overlap using Figure 1 as a reference.}
    	
        \begin{figure}[ht]
			\begin{center}
				\includegraphics{detector_layout}
				\caption{Figure showing each ESIS projection, in an array, with solar.  JDP working on it. \rts{Here is a preliminary version of this figure. I showed this to Charles and he was still in favor of the original version of this figure. We should talk about it.} \amy{I don't understand the arrows.  I think it would be more clear if there was a single arrow pointed north in the original image and the original image + octagon was black.  On the detector, to me it would make more sense if yellow -> red, and black or purple (the He line) went to blue and they each had a single arrow in the center pointed north.  Also good to list channels on this image.}}
...
\section{Data} 
    ...
    \subsection{Level-1 Data}
	    \begin{figure}
	    	...
	    	\caption{The top panel is the raw, Level-0 image captured by Channel \defaultChannel\ of ESIS during apogee. The bottom panel is the same image after Level-1 processing. \amy{ The Sun viewed through the octagonal field stop in the \ov spectral line is on the right side of the detector, this portion of the detector was cropped to generate Level-3 data.  The Sun viewed through the octagonal field stop in the \hei line is partially on the left side of the detector. The \mgxbright line is between and partially overlapping both of these two strong lines.}}
	    	\label{fig:L0_to_L1}
	    \end{figure}
    ...
    \subsection{Conversion to Photons and Despiking}
		...
		of \SI[per-mode=symbol]{19.6}{\electronvolt\per\photon} each count in electrons $\approx ?$ photons.
    ...
    \subsubsection{Vignetting correction}
        ...
        Each ESIS channel has a linear trend in intensity across the octagonal field of view along the dispersion direction due to internal vignetting in the optical system caused by \jdp{insightful comment here} \citep{ESIS}.
...
\section{Discussion/Conclusions and Future Work}
	...
	We use a Multiplicative Algebraic Reconstruction Technique (MART, Appendix \ref{MART}) to disentangle the combine spatial and spectral information in the four ESIS Level-3 images to find a single spatial-spectral cube ([x,y,$\lambda$]) for two small transient brightenings (Section \ref{sec:inversions}).
	...
	This especially noticeable in event d (Figure \ref{fig:l3_dif} and \ref{fig:other_x_inverted}) where the centroids of the blue and red jet are displaced by a little over an arcsecond.
	...
	This presentation is quite different from line profiles observed by IRIS \citep{depontieu2014} in \spectralline{Si}{iv}{1394}, where small transient events show smaller bulk flows, dominant line core emission, and excessive non-thermal broadening \jdp{probably best to cite a bunch of ee observations and not just IRIS}.
	...
	Several of these events were captured in \hei, \mgxbright, and \ov simultaneously, putting ESIS in a position to measure if the bi-modal nature of these line profiles is unique to \ov or persists at lower and higher temperatures in future work.
