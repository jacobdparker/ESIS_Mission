    ...
    \subsection{Level 3 Data}
    	...
    	\end{figure}
     	...
     	In order to use Level-3 data for early inversion the intensity in DN needs to  converted to photons in order properly account for uncertainty and then normalized between channels.
   		...
   		With an average quadrant gain, $Gain_q$, of $2.56\ electron\ eV^{-1}$ \rts{\SI[per-mode=symbol]{2.56}{\elementarycharge\per\electronvolt}}, a Silicon band gap energy of $3.6\ eV\ electron^{-1}$ \rts{\SI{3.6}{\electronvolt\per\elementarycharge}}, and $19.6\ eV\ photon^{-1}$ \rts{\SI[per-mode=symbol]{19.6}{\electronvolt\per\photon}} each count in $DN \approxeq 0.46$ photons in O V 627.8 \AA .
