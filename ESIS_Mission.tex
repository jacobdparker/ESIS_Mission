...
\section{Introduction}
    ...
    Slitless spectrograph data can be thought of as a projection of a \cck{three dimensional} spatial-spectral data cube, $I(x,y,\lambda)$, onto a two dimensional detector.  
    \cck{\sout{To invert temperature or density information from slitless spectrograph data, only a single projection of this cube is required, as demonstrated by \cite{winebarger2019}.}} To a limited extent, it proved possible to isolate individual line intensities for bright spectral features such as flare kernels using the Skylab data \citep{TBD}. Slitless spectrographs using two projections (usually one dispersed, and one undispersed) have proven sufficient to implement an efficient ground-based magnetograph \citep{deforest2004}, map Doppler shifts in the solar transition region \citep{Courrier2018}, and invert temperature or density information \citep{winebarger2019}. 
...
\section{Data} 
    ...
    \subsection{Level 3 Data}
    	...
    	Figure \ref{fig:coalign}a shows an example of a Level-3 image from Channel 2 \jpd{verify this} taken at \levthreetime.
