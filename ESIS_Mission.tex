...
\newcommand{\heii}{\spectralline{He}{ii}{304s}}
...
\section{Introduction}
    ...
    The solar transition region (TR) refers to the portion of the solar atmosphere that emits at temperatures between 20,000 K and 800,000 K degrees. 
    Initially, the transition region was viewed as simply the thin interface region between the dense, cool chromosphere and tenous, million-degree corona, where the temperature of the plasma dramatically increased two orders of magnitude over tens of kilometers. 
...
%\amy{Roy: can you change the text in Figure 2 to be "lights" instead of "signal"}. \rts{Changed it to `light frames' and also changed it to `upleg/downleg dark frames' to match.}
		...
		ESIS was launched at 18:04:00~UTC %\rts{\timeMissionStart~UTC} 
		...
		\begin{center}
			...
			\begin{tabular}{ll|ll}\hline
				Launch Date & \rts{\dateMission} & Image Size  & \rts{\imageShape~pix}\\
				Data Acquisition Time & \rts{\timeDataStart--\timeDataStop~UTC} & Avg. Noise: & \rts{\readoutNoise}\\
			    Pointing   &  $\approx$ (15\arcsec, -35\arcsec) & Avg. Gain &   \rts{\gain} \\
				Field of View  & $\approx 11.3$\arcmin octagonal  & Exposure Time & 10\,s \\
				Roll & $??? \pm ???^\circ$ CW & No. Light Exp: &\rts{\numDataFrames}\\
			    Spatial  Plate Scale  &  \rts{\plateScale} & No. Dark Exp: &\rts{\numDarkFrames} \\
				Spectral  Plate Scale  &  \rts{\dispersion} & \\
					\hline
			\end{tabular}
		\end{center}
		\end{table}
...
\section{Data} 
    ...
    Third, to retain the most information in the data, it is desired to included detailed mapping from the solar spectral cube to each unique detector instead of shifting, rotating, or cropping the detector data.  
    ...
    Instead, we seek a shortcut to interpretation of the ESIS data, which allows for taking differences between images and simplified inversions.
    ...
    \subsection{Level-1 Data}
	    ...
	    Each image was written to a fits files with on-board timestamp, camera number, and other parameters, such as the read out from temperature sensors included in the header.   For each channel, there were \numDataFrames\ light images that were processed into Level 1 data and \numDarkFrames\ dark images used to process the data.
    ...
    \subsection{Level 3 Data}
    	...
    	Level-3 data also allows for single wavelength inversion prior to the completion of the final optical distortion model and the Level-2 data product.  A description of both qualitative and quantitative analysis of Level-3 data is given in Section 4.  Generating Level-3 data products require several steps including 1) correction of the optical distortion and internal co-alignment of the four ESIS channels, 2) correction of each channel for vignetting, including identifying regions where the contributions of the overlapping \mgxbright line confuse the \ov data, and 3) correction for the relative radiometric differences in each channel.  Each of these corrections are described in detail below. 
  ...
  %\amy{We need an image of Level 1 data here, with the portion that is cropped/rotated to make 4a. It can just be part of 4 and and be above the two images or its own figure or we can include the cropped lines in the image I suggested in the previous section.}
  
  %\amy{Note that I added a comment about Level3 processing in the caption of Figure 4, maybe that suffices for the image I am referring too}
    ...
    \subsubsection{Optical distortion correction and channel co-alignmentment}
        ...
        First, each ESIS image is roughly cropped around the \ov spectral line, (roughly pixels 1100 - 2000 of the Level-1 data shown in Figure~\ref{fig:L0_to_L1}) and then co-aligned to the closest AIA 304\,\AA\ image in time.  
  ...
  \amy{This section is too detailed and not detailed enough simultaneously.  You are providing a lot of information, but not enough to understand why the details you are providing are important.  Let's talk about it.  I think adding a few explanatory sentences is all that is required.}
        ...
        Despite the uncertainties in vignetting, which we have found difficult to quantify, Level-3 differences are much flatter in intensity post vignetting correction as is seen in Figure \ref{fig:vig_correct}c, and will therefore lead to a higher fidelity intensity recovery when inverting Level-3 data.
          ...
          \jdp{make sense of this with level1}
  ...
  \amy{atmospheric absorption and despiking - mention somewhere}
