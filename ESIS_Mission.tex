...
\section{Introduction}
    ...
    Explosive events were discovered in rocket flights \citep{Dere1989}%    \sout{. Explosive event outflows are typically}
    \edit1{, and exhibit Doppler broadening} of order $\pm 100$\,km/s. 
...
%    \sout{Explosive events have been found to present in different ways and have been observed by many instruments since their original discovery.}
    ...
    This enhanced line core emission points to a larger amount of stationary plasma, and has been attributed to %\sout{the presence of a dominate} 
    \edit1{reconnection by the} tearing mode instability %\sout{during the reconnection} 
    \citep{Innes2015}.
    Enhanced emission in the line core of a larger explosive event viewed in \heii \ by the Multi-Order Solar Extreme-ultraviolet Spectrograph (MOSES)  has also been attributed to the tearing mode instability \citep{Fox2010}, \edit1{but} tens of smaller events in the same data \edit1{have} clear bi-modal profiles \citep{Rust2019}.
    ...
    In order to fully capture the velocity distribution and temporal evolution of quickly evolving solar events, particularly those with larger spatial extents, we \edit1{require imaging} instruments capable of gathering spectra in every pixel over a large field of view, at every exposure.
    ...
    ESIS builds off of the MOSES concept, but adds an additional projection %\sout{, the ability to independently focus each channel,} 
    and an explicit field of view defined by the field stop, a significant upgrade in flexibility and data quality over MOSES.
	...
	\begin{table}
		\begin{center}
			...
			\begin{tabular}{l|r|r}
				...
				\hline
%				\input{linelist_table_trim.tex}\\ Cheating for thesis purposes
				He\,{\sc i} & 584.33 & 0.70 \\
	...
	\end{table}
    	...
    	The ESIS optical design consists of a single parabolic primary mirror, an octagonal field stop placed at %\sout{an intermediate focal plane} 
    	\edit1{prime focus}, and 4 spherical diffraction gratings each with their own corresponding CCD detector.
    	...
    	The light that exits the field stop is reimaged  by \edit1{each of} four spherical diffraction gratings onto \edit1{its} own CCD.
    	...
    	Hence, each of these channels capture\edit1{s} a unique projection of the spatial spectral cube, shown in Figure \ref{fig:level_1_array}. 
    	...
    	\edit1{Simultaneously imaging with multiple slitless spectrographs, with different dispersion directions, is what makes ESIS a CTIS}. 
...
In fact, % \sout{the relationship between position and wavelength in each pixel depends on distortion parameters which we expect to vary subtly across the field of view} 
\edit1{the distortion is a nontrivial function of both position and wavelength \citep{ESIS}}, so that it cannot be undone \edit1{in two dimensions. It must be addressed by} simultaneously inverting the spatial-spectral cube across all wavelengths.
...
\edit1{As we have noted, ESIS images cannot be straightforwardly distortion corrected prior to distribution.
...
The Level 2 distortion model must incorporate the optical design parameters \edit1{and} the consequences of small element-by-element misalignments that inevitably occur when an optical system is built up.  
This model is in active development, \edit1{and is not} described further here.
...
Over the short wavelength range corresponding to a single spectral line (for our present purpose, we will choose \ov), \edit1{the wavelength dependence of the ESIS distortion function is negligible, and the spatial distortion can be corrected} by comparing ESIS data to co-temporal AIA 304\,\AA\ data. 
...
of 19.6\,\SI{}{\electronvolt}\edit1{, this} gives 0.18 photons per electron.
...
The co-alignment was achieved through a linear coordinate transformation of the cropped ESIS image that maximizes the zero lag cross-correlation between it and AIA 304\edit1{. T}he results of this are shown in Figure \ref{fig:coalign}.
\edit1{The mean correlation coefficient in this step is 0.26.}
...
\edit1{The mean correlation coefficient in this step is 0.92.}
...
We find that the internal alignment improves \edit1{both} co-alignment \edit1{with} the reference channel (dots in Figure \ref{fig:cc}) \edit1{and} the co-alignment between every other combination of channels (stars in Figure \ref{fig:cc}).
...
After the co-alignment is complete each Level 3 image has been re-binned to AIA resolution, 0.6 arcsecond per pixel, and can be assigned the WCS information \citep{WCS} from the \edit1{AIA image} nearest in time, providing pointing information and easier co-alignment with coordinating instruments.
\edit1{Image interpolation during co-alignment is performed using a third-order spline interpolation, the current default of scipy.ndimage.map\_coordinates.}
...
\edit1{The resulting final fit, $m_i = \vigfit$, differs from the field predicted using ray tracing and geometric optical models of the ideal instrument \citep{ESIS}.
...
Despite these issues our correction to the vignetting in Level 3 images makes their differences much flatter in intensity post correction as is seen in Figure \ref{fig:vig_correct}c, and will therefore lead to a higher fidelity intensity recovery when inverting Level 3 data.}
    ...
    \subsubsection{Relative Radiometric Correction }
        ...
        This normalizes the signal between each channel at every exposure \edit1{and} corrects for the \edit1{time-varying} atmospheric absorption mentioned in earlier sections. 
...
\section{Preliminary Results}
    ...
    \subsection{Level 3 Difference Images} \label{sec:dif_images} 
    	...
    	% Other things to 
  		...
  		\begin{figure}
  			\begin{interactive}{animation}{'4\_9.mp4'}
  			\centering
...
%    	 \sout{with lobes pointed downward in the difference between the Channel 2 and Channel 3 Level 3 image.
%    	Since we know that the direction of positive wavelength dispersion is up and to the right in Channel 2 and up and to the left in Channel 3 (Figure \ref{fig:mgx_overlap})} 
    	\edit1{By the reasoning given above,} we immediately know that this event is predominantly blue shifted.  
    	...
    	\begin{figure}
    		\begin{center}
    		\begin{interactive}{animation}{'4\_11.mp4'}
    			\includegraphics{main_event_dif}
    		\end{interactive}
    			\caption{The largest and most complex velocity event (e) %eruption
    				captured by ESIS shown at three different times. The event starts as two jets (one red, one blue) with slight spatial separation \edit1{at location e1}, evolves into a strong red shift where the event begins paired with faint blue shifted material above \edit1{(e2)}, and ends in a complicated combination of velocity not easily interpreted from difference images. 
%    				Co-temporal images from AIA 304\,\AA, 171\,\AA, and 193\,\AA\ are included in the \MYhref{https://scholarworks.montana.edu/xmlui/handle/1/16535}{animation} (Appendix \ref{ani}) for additional context.
					\edit1{The animation of this figure shows the complete evolution of the event with co-temporal images from AIA 304\,\AA, 171\,\AA, and 193\,\AA\ for additional context.
    ...
    \subsection{Early Inversions} \label{sec:inversions}
    	...
    	\begin{figure}
    		\begin{interactive}{animation}{'4\_13.mp4'}
%    		IF Thesis
%			\begin{center}
%				\resizebox{5.75in}{!}{% 			
%					\begin{tikzpicture}	
...
%    		\end{center}
    		...
    		\end{tikzpicture}
    		\end{interactive}
    		\caption{MART inverted results for event c in Figure \ref{fig:l3_dif} \edit1{at four different times}. 
...
%    			Full evolution included in \MYhref{https://scholarworks.montana.edu/xmlui/handle/1/16535}{animation} (Appendix \ref{ani}). 
    			\edit1{The animation of this figure shows the full temporal evolution of these line profiles measured by ESIS}    		
    		}
        ...
        \begin{figure}
        	\begin{interactive}{animation}{'4\_14.mp4'}
%        	IF Thesis
%			\resizebox{5.75in}{!}{%
%     			\begin{tikzpicture}
%					%begin by adding a node for each figure
...
%        	}
        	...
        	\end{tikzpicture}
        	\end{interactive}
    	\caption{MART inverted results for event d in Figure \ref{fig:l3_dif}, \edit1{at four select times}, presented in the same fashion as Figure \ref{fig:perfect_x_inverted}. 
%    	Full evolution included in \MYhref{https://scholarworks.montana.edu/xmlui/handle/1/16535}{animation} (Appendix \ref{ani}).
		\edit1{The animation of this figure shows the full temporal evolution of these line profiles measured by ESIS}
    	}
...
%    	In Figures \ref{fig:perfect_x_inverted} and \ref{fig:other_x_inverted}, and their associated animations (Appendix \ref{ani}), we present the MART inversion for two different compact transient events, labeled c and d in Figure \ref{fig:l3_dif}.
		...
		We performed a double Gaussian fit for each line profile in order to more easily pick out the location of each peak in \edit1{in the line profile and measured the associated Doppler shift of each component.}
		...
		This is visible both in the grid of line profiles and the difference images, \edit1{and} it is highlighted in Figure \ref{fig:otherx_timeseries}.
	    ...
	    \edit1{The described MART inversions yield very smooth line profiles.
        ...
        In particular, we consistently find line profiles consistent with a qualitative interpretation of the difference image movies, and measure bulk flows \edit1{(the shift in centroid of our fitted guassians)} within $\sim\pm$ 15\,km\,s$^{-1}$ ($\pm$ 1\,pixel) for a given exposure. 
...
\section{Discussion/Conclusions and Future Work}
	...
	It is a slitless imaging spectrograph, of which there only a handful of examples, \edit1{and} also a Computed Tomography Imaging Spectrograph (CTIS).  
	...
	\edit1{Moreover,} they reveal several larger and more complex events with large velocities, most notably the eruption identified near disk center (event e in Figure \ref{fig:l3_dif}).
    ...
    It is \edit1{likewise} puzzling that in both events c and d there is a lack of symmetry in velocity between the red and blue jets.
	...
	\edit1{This will} allow us to track energy and material moving through multiple layers of the solar atmosphere to form a clearer picture of where and how these reconnection events unfold. 
	...
	We believe this first flight of ESIS successfully demonstrated the viablility of CTIS style instruments for studying solar transients and that \edit1{the} solar physics community should continue to pursue the technology, and implement it in future missions. 
