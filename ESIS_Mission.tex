...
%		\end{center}
		...
		ESIS was launched at ????\rts{\timestart}~UT  on September 30, 2019 from White Sands Missile Range.  The target of observation was quiet sun at disk center.  The Solar Pointing and Aerobee Control System (SPARCS) maintained a constant target for the duration of the flight.  For ???\,s \rts{what time are we going for here, time above 150 km?}, ESIS recorded full detector ($\sim$2k$\times$1k) images with a 10\,s exposure and cadence. % Because the time on the Hi-C onboard DACS drifts, an adjustment of 126\,s was applied to all data headers in post-processing. 
...
\section{Data} 
    ...
    Level-1 data prepares raw CCD data for scientific work through a quadrant dependent bias and dark subtraction, gain correction, and de-spiking. 
    ...
    The highest level data product we envision providing in the future, Level 4, is the intensity as a function of $[x, y , \lambda]$, which will be the result of an inversion of the Level 2 data. \amy{Does it have to be level 2 data?  I would stop previous sentence after inversion.} Since inversion is not unique, there may be multiple Level 4 products obtained by different methods.  Level 4 data products will be described in future publications as they become available.  
    ...
    % 	[Copied from Hi-C paper as a reminder that we need to talk about atmospheric absorption somewhere.] We use the normalized total intensities of the Level~1.0 processed flight data (processing levels described in Section~\ref{sec:data}) to assess the relative atmospheric absorption of the signal as a function of flight time.  The transmission, shown in Figure~\ref{fig:absorb} in combination with the payload altitude, is calculated as the inverse of the relative absorption (i.e., (absorption coefficient)$^{-1}$). More than 4 minutes of data were unaffected by the atmosphere.  The atmospheric absorption was compensated for in the Level~1.5 processed data set by multiplying the images by their respective absorption coefficient.  These coefficients are provided in the header of this processed set.
    	...
    	\jdp{Normalize level-1 to account for atmospheric effects}
    ...
    \subsection{Level 3 Data}
    	...
    	Figure \ref{fig:coalign}a shows a final Level-3 image from Camera 1 taken at \levthreetime.
  		...
  		\begin{figure}[htb!]
    		...
    		\caption{Caption \rts{what happened to the y-axis ticks on the AIA image?}}
    		\label{fig:coalign}
    	\end{figure}
     	...
     	In order to use Level-3 data for early inversion the intensity in DN needs to  converted to photons in order properly account for uncertainty and then normalized between channels.
   		...
   		\end{equation}
   		\rts{Can we use a single letter for variable names? Right now it looks like $G \times a \times i \times n_q$ to me.}
   		With an average quadrant gain, $Gain_q$, of $2.56\ electron\ eV^{-1}$ \rts{\SI{2.56}{\elementarycharge\per\electronvolt}}, a Silicon band gap energy of $3.6\ eV\ electron^{-1}$ \rts{\SI{3.6}{\electronvolt\per\elementarycharge}}, and $19.6\ eV\ photon^{-1}$ \rts{\SI{19.6}{\electronvolt\per\photon}} each count in $DN \approxeq 0.46$ photons in O V 627.8 \AA .
   		...
   		Prior to co-alignment each AIA image was prepped to Level-1.5 using the aiapy routines \texttt{update\_pointing.py} and \texttt{register.py} \rts{Those functions are not located in modules by that name, you might just want to say \texttt{aiapy.calibrate.update\_pointing()}} \citep{aiapy}.
   		...
   		After the transformation each ESIS channel is re-binned to AIA resolution and can be assigned the WCS \rts{Do we have a WCS citation anywhere?} information from AIA Level 1.5 providing pointing information and easier co-alignment with other instruments.
        ...
        \end{equation}
       	...
       	\end{equation}
       	\rts{\begin{equation}
       		r = x_0 + [\cos(\alpha_i)(x-x_0-x_{\mathrm{drift}}*s) - \sin(\alpha_i)(y-y_0-y_{\mathrm{drift}}*s)].
       		\end{equation}}
...
\section{Preliminary Results}
	...
	Simple events viewed in Level-3 difference images are easily interpretable \amy{is this a word} with little insight into how the ESIS \amy{are you missing a word?} and therefore serve as a sanity check for more complicated inversions. 
	\amy{suggested re-write for previous sentence:  Simple events viewed in Level 3 difference images can be understood qualitatively and therefore serve as a sanity check for more complicated inversions.}
    ...
    \subsection{Level-3 Difference Images}
    	...
    	\begin{figure}[htb!]
    		...
    		\caption{An example of a red, blue, and broadened transient brightenings. \jdp{Think about adding back in the camera 2 and 3 images}}
